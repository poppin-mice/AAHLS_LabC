...
`define AUTOTB_CLOCK_PERIOD_DIV2 2.40

`define AESL_DEPTH_src 1
`define AESL_DEPTH_dst 1
...
parameter LATENCY_ESTIMATION = 16822;
parameter LENGTH_src = 17000;
parameter LENGTH_dst = 17000;
...
reg [31:0] ap_c_n_tvin_trans_num_src;
...
begin
    ...
    initial begin : proc_gen_axis_internal_ready_src
        ...
        forever begin
            @ (ap_c_n_tvin_trans_num_src or src_transaction);
            if (ap_c_n_tvin_trans_num_src > src_transaction) begin
                src_ready_reg = 1;
            end else begin
                src_ready_reg = 0;
            end
    ...
    `define STREAM_SIZE_IN_src "../tv/stream_size/stream_size_in_src.dat"
    
    initial begin : gen_ap_c_n_tvin_trans_num_src
        integer fp_src;
        reg [127:0] token_src;
        integer ret;
        
        ap_c_n_tvin_trans_num_src = 0;
        end_src = 0;
        wait (AESL_reset === 1);
        
        fp_src = $fopen(`AUTOTB_TVIN_src, "r");
        if(fp_src == 0) begin
            $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_src);
            $finish;
        end
        read_token(fp_src, token_src); // should be [[[runtime]]]
        if (token_src != "[[[runtime]]]") begin
            $display("ERROR: token_src != \"[[[runtime]]]\"");
            $finish;
        end
        ap_c_n_tvin_trans_num_src = ap_c_n_tvin_trans_num_src + 1;
        read_token(fp_src, token_src); // should be [[transaction]] or [[[/runtime]]]
        if (token_src == "[[[/runtime]]]") begin
            $fclose(fp_src);
            end_src = 1;
        end else begin
            end_src = 0;
            read_token(fp_src, token_src); // should be transaction number
            read_token(fp_src, token_src);
        end
        while (token_src == "[[/transaction]]" && end_src == 0) begin
            ap_c_n_tvin_trans_num_src = ap_c_n_tvin_trans_num_src + 1;
            read_token(fp_src, token_src); // should be [[transaction]] or [[[/runtime]]]
            if (token_src == "[[[/runtime]]]") begin
                $fclose(fp_src);
