...
//all condition in one loop
    ...
    for(int i = 0; i < 128; i++){
        ...
        for(int j = 0; j < 128; j++){
            ...
            img_buffer[516] = src[259+i*128+j];

            //NMS (starts when input >= 130)
            
            // if(theta[0] < 0) theta[0] += 3.1415926535;
            float q = 65025.0; float r = 65025.0;
            if(theta[0] > -0.414213){//region 1 4 3
                if(theta[0] > 0.414213){//region 4 3
                    if(theta[0] > 2.414213){//region 3
                        q = G[256 + 1];
                        r = G[0 + 1];
                    }
                    else{//region 4
            ...
            if(i*128 + j >= 129){    
                dst[(i<<7) + j - 129] = (sup >= 6400)? 255 : ((sup > 900)? 127 : 0);
            }
        }
    }
    rest_result:
    for(int i = 16384-129; i < 16384; i++){
        rest_G_shift:
        for(int k = 0; k < 258; k++){
            G[k] = G[k+1];
            // theta[k] = theta[k+1];
        ...
        int sup = (G[128 + 1] >= q && G[128 + 1] >= r)? (int)G[128 + 1] : 0;
        dst[i] = (sup >= 6400)? 255 : ((sup > 900)? 127 : 0);
    }
}
