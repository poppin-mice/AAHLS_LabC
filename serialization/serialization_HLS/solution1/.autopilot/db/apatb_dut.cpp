...
{
transaction<8> tr(240000);
aesl_fh.read(AUTOTB_TVOUT_PC_gmem, tr.p, tr.tbytes);
if (little_endian()) { tr.reorder(); }
tr.send<1>((char*)__xlx_apatb_param_src_buff, 120000, 0);
tr.send<1>((char*)__xlx_apatb_param_dst_buff, 120000, 120000);
}
#else
try {
...
if (__xlx_apatb_param_src_buff) {
  tr.import<1>((char*)__xlx_apatb_param_src_buff, 120000, 0);
}
__xlx_offset_byte_param_dst_buff = 120000*1;
if (__xlx_apatb_param_dst_buff) {
  tr.import<1>((char*)__xlx_apatb_param_dst_buff, 120000, 0);
}
